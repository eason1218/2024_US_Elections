...
ggplot(data.frame(mu = mu_post), aes(x = mu)) +
  ...
  theme_minimal()
```
@fig-figure1 shows the posterior estimate for the parameter mu shows that the posterior mean is -0.5725. The 95% credible interval for μ ranges from -1.4148 to 0.5035, indicating the range within which the true value of μ is likely to fall with 95% probability.
```{r, fig.width=6, fig.height=3, fig.align='center'}
#| label: fig-figure2
#| warning: false
#| message: false
...
```
\newpage
@fig-figure2 shows the parameter sigma, the posterior mean is estimated to be 3.5336. The 95% credible interval for σ extends from 0.8571 to 10.2785, reflecting the uncertainty around the estimate and providing a range of probable values for σ.
...
```
@tbl-table1 show trump is the most likely candidate to win with an average percentage of 35.52.
